...
class UserAPITest(unittest.TestCase):
    ...
    def get(self, c, base):
        ...
        """
        result_data = c.search(base, tldap.SCOPE_BASE)
        no_results = len(result_data)
        if no_results < 1:
            raise tldap.NO_SUCH_OBJECT()
    ...
    def test_transactions(self):
        ...
        with tldap.transaction.commit_on_success():
            ...
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_REPLACE, "sn", "Gates") ])
            c.rollback()
        self.assertRaises(tldap.NO_SUCH_OBJECT, self.get, c, "uid=tux, ou=People, dc=python-ldap,dc=org")
        ...
        with tldap.transaction.commit_on_success():
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_DELETE, "telephoneNumber", None) ])
        ...
        with tldap.transaction.commit_on_success():
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_ADD, "telephoneNumber", "111") ])
        ...
        with tldap.transaction.commit_on_success():
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_ADD, "telephoneNumber", "111") ])
            self.assertRaises(tldap.TYPE_OR_VALUE_EXISTS, c.modify, "uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_ADD, "telephoneNumber", "111") ])
            self.assertEqual(self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['telephoneNumber'], [ "111" ])
        self.assertEqual(self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['telephoneNumber'], [ "111" ])

        # test search scopes
        c.add("ou=Groups, dc=python-ldap,dc=org", [ ("objectClass", ["top","organizationalunit"]) ])
        r = c.search("uid=tux, ou=People, dc=python-ldap,dc=org", tldap.SCOPE_BASE, "uid=tux")
        self.assertEqual(len(r), 1)
        r = c.search("ou=People, dc=python-ldap,dc=org", tldap.SCOPE_BASE, "uid=tux")
        self.assertEqual(len(r), 0)
        r = c.search("dc=python-ldap,dc=org", tldap.SCOPE_BASE, "uid=tux")
        self.assertEqual(len(r), 0)
        r = c.search("ou=Groups, dc=python-ldap,dc=org", tldap.SCOPE_BASE, "uid=tux")
        self.assertEqual(len(r), 0)
        self.assertRaises(tldap.NO_SUCH_OBJECT, c.search, "dc=python,dc=org", tldap.SCOPE_BASE, "uid=tux")

        r = c.search("uid=tux, ou=People, dc=python-ldap,dc=org", tldap.SCOPE_ONELEVEL, "uid=tux")
        self.assertEqual(len(r), 1)
        r = c.search("ou=People, dc=python-ldap,dc=org", tldap.SCOPE_ONELEVEL, "uid=tux")
        self.assertEqual(len(r), 1)
        r = c.search("dc=python-ldap,dc=org", tldap.SCOPE_ONELEVEL, "uid=tux")
        self.assertEqual(len(r), 0)
        r = c.search("ou=Groups, dc=python-ldap,dc=org", tldap.SCOPE_ONELEVEL, "uid=tux")
        self.assertEqual(len(r), 0)
        self.assertRaises(tldap.NO_SUCH_OBJECT, c.search, "dc=python,dc=org", tldap.SCOPE_BASE, "uid=tux")

        r = c.search("uid=tux, ou=People, dc=python-ldap,dc=org", tldap.SCOPE_SUBTREE, "uid=tux")
        self.assertEqual(len(r), 1)
        r = c.search("ou=People, dc=python-ldap,dc=org", tldap.SCOPE_SUBTREE, "uid=tux")
        self.assertEqual(len(r), 1)
        r = c.search("dc=python-ldap,dc=org", tldap.SCOPE_SUBTREE, "uid=tux")
        self.assertEqual(len(r), 1)
        r = c.search("ou=Groups, dc=python-ldap,dc=org", tldap.SCOPE_SUBTREE, "uid=tux")
        self.assertEqual(len(r), 0)
        self.assertRaises(tldap.NO_SUCH_OBJECT, c.search, "dc=python,dc=org", tldap.SCOPE_BASE, "uid=tux")

        # test replacing attribute with rollback
        with tldap.transaction.commit_on_success():
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_REPLACE, "telephoneNumber", "222") ])
        ...
        with tldap.transaction.commit_on_success():
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_DELETE, "telephoneNumber", "222") ])
            self.assertRaises(KeyError, lambda: self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['telephoneNumber'])
            c.fail() # raises TestFailure during commit causing rollback
            self.assertRaises(tldap.exceptions.TestFailure, c.commit)
        self.assertEqual(self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['telephoneNumber'], [ "222" ])
        ...
        with tldap.transaction.commit_on_success():
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_DELETE, "telephoneNumber", "222") ])
            self.assertRaises(tldap.NO_SUCH_ATTRIBUTE, c.modify, "uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_DELETE, "telephoneNumber", "222") ])
            self.assertRaises(KeyError, lambda: self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['telephoneNumber'])
        self.assertRaises(KeyError, lambda: self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['telephoneNumber'])

        # test success when 3rd statement fails; need to roll back 2nd and 1st statements
        with tldap.transaction.commit_on_success():
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_REPLACE, "sn", "Milkshakes") ])
            self.assertEqual(self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['sn'], [ "Milkshakes" ])
            c.modify("uid=tux, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_REPLACE, "sn", "Bannas") ])
            self.assertEqual(self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['sn'], [ "Bannas" ])
            self.assertRaises(tldap.ALREADY_EXISTS, c.add, "uid=tux, ou=People, dc=python-ldap,dc=org", modlist)
            c.fail() # raises TestFailure during commit causing rollback
            self.assertRaises(tldap.exceptions.TestFailure, c.commit)
        self.assertEqual(self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['sn'], [ "Gates" ])
        ...
        with tldap.transaction.commit_on_success():
            ...
            c.modify("uid=tuz, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_REPLACE, "sn", "Tuz") ])
            self.assertEqual(self.get(c, "uid=tuz, ou=People, dc=python-ldap,dc=org")['sn'], [ "Tuz" ])
            c.fail() # raises TestFailure during commit causing rollback
            self.assertRaises(tldap.exceptions.TestFailure, c.commit)
        self.assertEqual(self.get(c, "uid=tux, ou=People, dc=python-ldap,dc=org")['sn'], [ "Gates" ])
        self.assertRaises(tldap.NO_SUCH_OBJECT, self.get, c, "uid=tuz, ou=People, dc=python-ldap,dc=org")
        ...
        with tldap.transaction.commit_on_success():
            c.modify("uid=tuz, ou=People, dc=python-ldap,dc=org", [ (tldap.MOD_REPLACE, "sn", "Gates") ])
